...
class WebViewController: UIViewController, OSSubscriptionObserver, GADBannerViewDelegate, SKStoreProductViewControllerDelegate,SKProductsRequestDelegate, SKPaymentTransactionObserver, FBInterstitialAdDelegate, GADFullScreenContentDelegate
{
    ...
    override func viewDidLoad()
    {
        ...
        if (splashScreenEnabled) {
            ...
            // Zeige einfach weißen Text "DocumentDiary" an
            bgView.isHidden = true
            
            // Erstelle ein Label für den Text
            let textLabel = UILabel()
            textLabel.text = "DocumentDiary"
        ...
        // WICHTIG: Konfiguriere AVAudioSession für Speech Recognition
        let audioSession = AVAudioSession.sharedInstance()
        
        do {
            // Verwende .playAndRecord für Speech Recognition (ermöglicht Mikrofon-Zugriff)
            // Mit .defaultToSpeaker wird Audio über Lautsprecher ausgegeben, nicht über Receiver
            ...
            print("AVAudioSession Konfigurationsfehler: \(error)")
            // Fallback auf Playback falls Record nicht möglich
            do {
                try audioSession.setCategory(.playback)
                try audioSession.setActive(true)
            } catch {
                print("AVAudioSession Fallback fehlgeschlagen: \(error)")
            }
        }
        ...
        // WICHTIG: Erlaube Media Capture (Mikrofon) für Speech Recognition
        // Diese Einstellungen sind bereits oben gesetzt, aber wir stellen sicher, dass sie korrekt sind
    ...
    func setupCustomNavigationBar() {
        ...
        // DocumentDiary Hauptfarbe: #2c5a61
        navBar.backgroundColor = UIColor(red: 44/255.0, green: 90/255.0, blue: 97/255.0, alpha: 1.0)
        containerView.addSubview(navBar)
        
        // Title Label
        let titleLabel = UILabel()
        titleLabel.text = "DocumentDiary"
